...
When Prettier identifies cases where it looks like you've placed some code it knows how to format within a string in another file, it will by default try to format that code with a tag named `html` in JavaScript or in Markdown code blocks. 

To enable this feature in <strong>JavaScript</strong> you need to add an [identity tag](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/raw?retiredLocale=de#building_an_identity_tag) named `html`. 
